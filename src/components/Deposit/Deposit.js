            ...
            if(isAppr)
            {
                await exportedObject.MY_CONTRACT.methods.deposit(amountDep).estimateGas({from: userAddress}).then(async function(gasAmount){
                    ...
                    else
                    {
                        ...
                        await exportedObject.MY_CONTRACT.methods.deposit(amountDep).send({ from: userAddress, gas: gasLimit, value: 0 }).on('transactionHash', function(hash){
                            MySwal.fire({
                                position: 'center',
                                icon: 'success',
                    ...
                    else
                    {
                        await exportedObject.MY_CONTRACT.methods.deposit(amountDep).estimateGas({from: userAddress}).then(async function(gasAmount){
                            ...
                            else
                            {
                                gasLimit = exportedObject.web3.utils.toHex(exportedObject.web3.utils.toWei('0.00083026', 'gwei'));
                                estimateGas = exportedObject.web3.utils.toHex(exportedObject.web3.utils.toWei('2.5', 'gwei'));
                                await exportedObject.MY_CONTRACT.methods.deposit(amountDep).send({ from: userAddress, gasPrice: estimateGas, gas: gasAmount }).on('transactionHash', function(hash){
                                    Swal.fire({
                                        position: 'center',
                                        icon: 'success',
                                        title: `Deposit Request Submittted`,
        ...
        <div className='row'>
          <div className='col-12 col-md-12'>
            <div className='text-center'>
              ...
              <h5 className='mt-3 mb-3'><span className='badge badge-info'>Login Account: {userAddress}</span></h5>
            </div>
            <div className='row'>
                  ...
                  <table className="table table-borderless">
                    <tbody>
                      <tr>
                        ...
                        <td className="text-center">=</td>
                        <td className="text-center"><span className="total">{parseFloat(depositAmount) + parseFloat(depositAmount * 0.16)}</span> DAI</td>
                      </tr>
                      <tr>
                        <td>Deposit</td>
                        <td className="text-center">&nbsp;</td>
